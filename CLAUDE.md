...
- [ ] Wall entry added on library.html: live demo of every variant and state, variant and state chips, exact snippet with copy button — in the entry structure (description · variants · preview · customization · code · AI instructions).
...
- Fonts are role tokens too: seven faces, one per job — `--font-heading` (calligraphic serif), `--font-body` (rounded sans), `--font-mono` (technical monospace, also the always-caps eyebrow voice at `--weight-eyebrow`), `--font-quote` (editorial serif, italic, for quotes and callouts), `--font-reading` (editorial serif, upright, for long-form article text), `--font-numeric` (display serif for big standalone numbers), `--font-hand` (handwritten pen for margin notes and annotations, never body text). The Default theme loads them via a single `@import` from Google Fonts at the top of `facet.css` — an accepted external dependency (no extra HTML tag, no build step, still one CSS + one JS), and every stack ends in a system fallback so text renders instantly and survives the fonts failing to load. A theme swaps the faces; components only ask for a role. New font roles are added only when a face does a genuinely different job — never one per component.
