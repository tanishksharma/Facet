...
Facet is a plain HTML, CSS and JS design library used across everything: apps, websites, pitch decks, documents and business cards. It is one CSS file and one JS file (plus two small optional engines: `facet-sw.js` for the PWA layer, and `gem.js` — the animated SVG gemstone renderer, the library's signature ornament and the source of the site's own brand mark) hosted at a public URL. Any project pulls it in with one link tag and one script tag. No React, no build step, no npm install, never minified.
...
- [ ] Wall entry added on library.html: live demo of every variant and state, variant and state chips, exact snippet with copy button — in the entry structure (description · variants · preview · customization · code · AI instructions). Its h3 carries the status tag `data-status="dev"` — never "live"; only the maintainer promotes an entry.
...
- Pager law (amended 30 Aug 2026): full-page snap sections that can OUTGROW the viewport are never built with CSS scroll-snap — mandatory snap re-snaps tall sections and wedges the next gesture on iOS; the spring pager (.snap upgraded to .is-paged) is the only thing that survives there. Sections that all FIT the viewport (app folds whose scrolling happens in nested scrollers) are the case native CSS snap handles well, and there it beats the pager: iOS gesture latching hands a swipe from an inner scroller to the fold container only on a NEW gesture, so reaching a list's edge never flings the page — facet.js picks this native mode (.is-snapped) automatically when every section fits. Corollary: inner scrollers inside a native-snap fold must NOT set `overscroll-behavior: contain` — it blocks the gesture handoff entirely and the fold becomes unpageable from that pane; leave chaining native and let latching do the work.
...
- Riser-transform law: a `.sheet-rise` panel is placed by its own CORNER (top/bottom plus left/right) and must keep `transform: none`. The edge sheet's spring writes `translate(x, -50%)` on every render, so any code path that renders a riser yanks it up by half its height. The path that bit us was the resize handler: Safari fires a resize when you return to the tab, so an open panel had silently jumped upward while you were away. Anything that re-renders an open sheet checks `riser()` first.
- Tap-flash law: iOS paints a grey rectangle over anything tappable the instant a finger lands, over the whole box and ignoring the element's own radius — so a bare button flashes as a hard-edged slab. Every control here already shows its own `:active` state, so the base layer clears `-webkit-tap-highlight-color` on the interactive elements, in `:where()` so an author who wants it back just declares it.
...
- Deployment default: whenever you build a feature, push it to the `staging` branch (Vercel deploys every branch — share the staging preview link). Live (from `main`): **https://facet.tanishksharma.com** · Staging (from `staging`): **https://staging-facet.vercel.app** (a vercel.app domain pinned to the `staging` branch). `main` stays the live site; merge staging into main when the user says to ship. Do NOT create extra feature branches unless the user explicitly asks for one. This overrides any older push-straight-to-main or branch-per-change workflow.
...
- The accent ranks are a PLACEMENT model, not an importance ladder (decided 14 Jul 2026): the three ranks are layers read base to top. `--accent-3` is the BASE layer the product sits in — page and section washes, pills and chips, the selected tab, the nav "you are here" state, focus rings, text-selection highlights, sliders (one hue at three strengths: `-wash` the ground, `-tint` a pill on a card, full for focus/ink). `--accent-2` is the MIDDLE layer that breaks information down — eyebrows, callouts, captions (support text and quiet fills, its `-text` defaulting to the neutral muted ink). `--accent-1` is the TOP layer, the highlight — the main headings (h1/h2 + the display roles), links, the one primary action, and the committed marks on inputs (the checkbox check, the radio dot, the switch) since those sit highest in hierarchy like a primary action. Onboarding a brand: base colour → accent-3, highlight → accent-1, support hue (only if a third exists) → accent-2; a single-colour product sets all three equal and lets placement carry the difference. Each rank derives `-tint` / `-text` / `-edge` (and `-wash` on the base) once in `:root` via color-mix, so a theme that re-sets only the three base hues gets the whole set free; pills, chips, badges and callouts carry the base hue as a very pale `-tint` fill with a normal-colour `-edge` outline (never a solid block), since washes are pale by design; `--accent-1-text` is the AA-safe writing tone, deepened per theme (velvet/elegant) where the fill gold fails AA at small sizes. This REPLACES the old "primary action / secondary fill / quiet interactive" reading of the ranks. Neutral controls carry NO brand colour: they read the new `--fill` / `--fill-hover` / `--fill-strong` tokens (aliased to the neutral surface ramp) — secondary and ghost buttons, the file button, table/dropdown/pagination/stepper hovers, tracks, placeholder avatars, spinners/skeletons — so a coloured theme keeps its controls calm and spends colour only in the three expressive layers. Reading text stays on the neutral text ramp; ordinary grey text is never accent-2 (only the deliberate eyebrow/callout/caption labels are). (Shipped complete 14 Jul 2026 across facet.css: token layer, `--fill` neutrals, and every consumer wired — headings, links, form marks and primary action → accent-1; eyebrows/callouts/captions → accent-2; pills, badges, chips, nav-current, mark, ::selection, washes → accent-3. A multi-brand comparison lab (Default / single-colour / two two-colour brands, each with a Ground·Pill·Base·Top swatch legend) now lives in the library.html Color entry, styled in docs.css `.accent-lab`. Still open as a board row: add a real `.callout` component for the middle layer, a Themes-page multi-colour demo, and a final AA sweep of all 8 theme/mode combos.)
...
- Entry status tags (decided 11 Jul 2026): every library.html wall entry's h3 carries `data-status="live"` or `data-status="dev"`. Live means the maintainer has personally reviewed and validated the piece; dev (rendered "in development") is everything else. A new entry always ships as dev; only the maintainer flips one to live — an AI never promotes an entry on its own judgment. The tag is docs chrome (styled in docs.css), not a library capability, so it has no llms.txt entry.
...
- Snap has two modes, chosen by section shape (decided 30 Aug 2026, from the Schedule Clock fold rework): a full-viewport .snap auto-upgrades to NATIVE mode (.is-snapped — CSS scroll-snap stays in charge; facet.js only adds `facet:section` events, the same element.facetPager API over scrollTo, snap feedback, hidden scrollbar) when every .snap-section fits the viewport, and to the SPRING PAGER (.is-paged, unchanged) when any section can outgrow it. `data-pager="snap"` and `data-pager="spring"` force a mode; `data-pager="off"` still leaves the markup untouched. The scroll gauge remains a pager-mode feature. This encodes the amended Pager law above; the reference feel is the main site's card strips (container-scoped native x-snap) applied to y folds.
