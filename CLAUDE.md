...
- The accent ranks are a PLACEMENT model, not an importance ladder (decided 14 Jul 2026): the three ranks are layers read base to top. `--accent-3` is the BASE layer the product sits in — page and section washes, pills and chips, the selected tab, focus and selected states, sliders (one hue at three strengths: `-wash` the ground, `-tint` a pill on a card, full for focus/ink). `--accent-2` is the MIDDLE layer that breaks information down — eyebrows, callouts, captions (support text and quiet fills, its `-text` defaulting to the neutral muted ink). `--accent-1` is the TOP layer, the highlight — the main headings (h1/h2 + the display roles) and the one primary action. Onboarding a brand: base colour → accent-3, highlight → accent-1, support hue (only if a third exists) → accent-2; a single-colour product sets all three equal and lets placement carry the difference. Each rank derives `-tint` / `-text` (and `-wash` on the base) once in `:root` via color-mix, so a theme that re-sets only the three base hues gets the whole set free. This REPLACES the old "primary action / secondary fill / quiet interactive" reading of the ranks; neutral controls (secondary buttons, table/dropdown hovers, steppers) read the neutral surface tokens, not an accent, so a coloured theme keeps its controls calm and spends colour only in the three expressive layers. (Shipped first slice 14 Jul 2026: token layer + model; headings→accent-1, eyebrow→accent-2. Still open, tracked as a board row: rewire pills/chips/tab-selection/section-wash to accent-3, add a `.callout` component for the middle layer, reroute the remaining neutral-hover consumers off accent-2, and per-theme AA verify all 8 theme/mode combos.)
