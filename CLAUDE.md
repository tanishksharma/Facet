...
- Native first — never rebuild what the platform ships (decided 16 Jul 2026): if HTML or CSS has a native mechanism for a behaviour, the library and everything built on it uses it; hand-rolled JS re-implementations are rejected in review. The canonical cases: a carousel is CSS scroll snapping on an overflow row (.card-row / the promo rail — never slide logic or translate math; the pager law stays the one exception, full-page vertical snap on iOS only), disclosure is `<details>`, modals are `<dialog>`, transient panels are the popover attribute. The consumer-facing version ships as the "Native first" guidance entry in llms.txt's Build advice. Standing follow-up (board row, Later): audit the HTML/CSS standard for every native capability the library should adopt.
- Feature planning base (decided 16 Jul 2026): during the documentation restructure, the Notion Feature inventory page (on the Facet hub) is the PLANNING base — the exhaustive, categorized, importance-ordered list (categories by weight, each opening with its core features, shipped vs planned marked) that the split documentation is rewritten from. llms.txt + the commented source stay the runtime truth consumers read until the split ships, and the keep-in-sync contract still applies to every commit in between.
