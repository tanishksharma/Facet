...
- The entry structure (decided 9 Jul 2026): every wall entry reads, in order — title · description (plain words only: what it is, no class names) · variant row (when variants exist) · PREVIEW · customization rows (each chip row labeled by its aria-label) · code (live, following the visible variant) · AI instructions (.ai-notes — the technical text: classes, attributes, knobs, rules; this is where the facet.css file-comment text lives on the wall). docs.js enforces preview-before-options for older markup.
