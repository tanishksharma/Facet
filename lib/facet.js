...
/* (docs mirror — identical to llms.txt · Description tooltip)
   ...
   and the text appears in a small bubble with a tail pointing at it.
   Placement measures the screen on every show: above the element when
   there is room, below when above would clip, slid sideways from a screen
   edge with the tail still pointing at the element, and beside it when a
   corner allows neither — never clipped, never over the safe areas, never
   blocking what is under it. On desktop it shows on hover after a short
   beat and on keyboard focus at once; clicking the element hides it
   instantly — the visitor has acted, the hint gets out of the way — and it
   does not return until the pointer leaves and re-enters. On touch screens
   a tap opens it alongside the element's own action, never instead of it,
   and any next interaction closes it: the first scroll, a tap anywhere
   else, the element opening something of its own (a sheet, a menu, a
   dialog), turning the device, or on its own after a few seconds. A
   disabled control still shows its bubble — often the explanation for why
   it is disabled. What it is for: explaining every interactive element in
   place — one of the library's core principles. The description doubles as
   documentation anyone can read by pointing at the thing. When to use it:
   every interactive element ships with one. data-tip-below steers only the
   JS-off fallback bubble; with facet.js running, placement is automatic. */
/* Tooltips — the JS half of the description tooltip. One floating bubble
   serves the whole page, created fresh and measured against the viewport
   plus the safe-area insets on EVERY show (panels, risers and scrolled
   pills move their triggers, so a wire-up-time measurement would lie):
   above the trigger first, below when above would clip, slid along a
   screen edge with the tail still anchored to the trigger's centre, and
   beside the trigger when a corner allows neither above nor below.
   Desktop (mouse or pen): hover shows it after ~300ms, keyboard focus at
   once; it hides on pointer leave, blur, Escape — and on CLICK of the
   trigger, because acting means the hint has done its job — and does not
   re-show until the pointer has left and re-entered. Touch: a tap shows
   it alongside the trigger's own action, and any next interaction hides
   it — the first scroll, a tap elsewhere, the trigger opening something,
   an orientation change, or a ~4s timeout. Only one bubble ever exists;
   showing a new one removes the old one first, no fade queue. Delegated,
   so late-added markup needs no wiring; the bubble never captures pointer
   events. While this runs the pure-CSS hover bubble stands down
   (data-tip-js on <html>); it remains the JS-off fallback. */
function initTooltips() {
  // the measured bubble takes over; the CSS fallback bubble stands down
  document.documentElement.setAttribute("data-tip-js", "");
  const HOVER_DELAY = 300;   // hover waits a beat; keyboard focus is instant
  const TOUCH_LIFE = 4000;   // the tap bubble's backstop timeout
  let bubble = null, current = null, pinned = false;
  let hoverEl = null, hoverTimer = 0, lifeTimer = 0;
  let cooled = null;         // clicked trigger: no re-show until the pointer leaves it
  let lastPointerType = "";  // click events carry no pointerType in Safari
  const hide = () => {
    clearTimeout(hoverTimer);
    clearTimeout(lifeTimer);
    hoverEl = null;
    if (bubble) bubble.remove();   // gone at once — no fade-out queue
    bubble = null;
    current = null;
    pinned = false;
  };
  // measure trigger and bubble against the viewport (minus the safe-area
  // insets the bubble's CSS exposes) and pick the spot: above, else
  // below, else beside; slid along an edge when centring would clip,
  // with the tail (--tip-arrow) still anchored to the trigger's centre
  const place = (el) => {
    const r = el.getBoundingClientRect();
    const b = bubble.getBoundingClientRect();
    const cs = getComputedStyle(bubble);
    const inset = (side) => (parseFloat(cs.getPropertyValue("--tip-safe-" + side)) || 0) + 8;
    const top = inset("top"), bottom = inset("bottom"),
          left = inset("left"), right = inset("right");
    const GAP = 10, TAIL = 12;   // TAIL keeps the tail clear of the bubble's corners
    const cx = r.left + r.width / 2, cy = r.top + r.height / 2;
    let side = "above", x, y = r.top - b.height - GAP;
    if (y < top) { side = "below"; y = r.bottom + GAP; }
    if (side === "below" && y + b.height > innerHeight - bottom) {
      // a screen corner: neither above nor below fits — go beside,
      // whichever side has the room
      side = r.left - b.width - GAP >= left ? "left" : "right";
      x = side === "left" ? r.left - b.width - GAP : r.right + GAP;
      x = Math.min(Math.max(x, left), innerWidth - b.width - right);
      y = Math.min(Math.max(cy - b.height / 2, top), innerHeight - b.height - bottom);
      bubble.style.setProperty("--tip-arrow", Math.min(Math.max(cy - y, TAIL), b.height - TAIL) + "px");
    } else {
      // centred on the trigger, slid sideways until it fits on screen —
      // the tail stays on the trigger even when the bubble has slid
      x = Math.min(Math.max(cx - b.width / 2, left), innerWidth - b.width - right);
      bubble.style.setProperty("--tip-arrow", Math.min(Math.max(cx - x, TAIL), b.width - TAIL) + "px");
    }
  ...
  const show = (el, pin) => {
    ...
    hide();                        // one bubble ever — the old one leaves first
    bubble = document.createElement("div");
    bubble.className = "tip-bubble";
    bubble.setAttribute("role", "tooltip");
    bubble.textContent = el.dataset.tip;
    document.body.appendChild(bubble);
    place(el);                     // re-measured on every show, never cached
    requestAnimationFrame(() => { if (bubble) bubble.classList.add("is-open"); });
    current = el;
    pinned = pin;
    if (pin) lifeTimer = setTimeout(hide, TOUCH_LIFE);
  };
  ...
  // hover (mouse or pen — a tablet's trackpad cursor counts): show after
  // a beat, so a pointer passing through doesn't flash bubbles
  document.addEventListener("pointerover", (e) => {
    if (e.pointerType !== "mouse" && e.pointerType !== "pen") return;
    const el = tipOf(e);
    if (!el) { clearTimeout(hoverTimer); hoverEl = null; if (!pinned) hide(); return; }
    if (el === current || el === cooled || el === hoverEl) return;
    clearTimeout(hoverTimer);
    hoverEl = el;
    hoverTimer = setTimeout(() => { hoverEl = null; show(el, false); }, HOVER_DELAY);
  }, { passive: true });
  document.addEventListener("pointerout", (e) => {
    if (e.pointerType !== "mouse" && e.pointerType !== "pen") return;
    const el = tipOf(e);
    if (!el || (e.relatedTarget && el.contains(e.relatedTarget))) return;
    if (el === cooled) cooled = null;   // left — the trigger may hint again
    if (el === hoverEl) { clearTimeout(hoverTimer); hoverEl = null; }
    if (el === current && !pinned) hide();
  }, { passive: true });
  // keyboard focus shows at once; blur hides
  ...
  document.addEventListener("click", (e) => {
    const el = tipOf(e);
    if (!el) { hide(); return; }        // a click or tap anywhere else closes
    const touch = (e.pointerType || (e.detail ? lastPointerType : "")) === "touch";
    if (!touch) { hide(); cooled = el; return; }  // acted — the hint gets out of the way
    if (opensSomething(el)) { hide(); return; }   // act, don't linger over what opened
  ...
  // It is also where a DISABLED control's tap bubble opens: disabled
  // controls swallow click but still dispatch pointer events, and their
  // tip is often the explanation for why they are disabled.
  document.addEventListener("pointerdown", (e) => {
    lastPointerType = e.pointerType;
    const el = tipOf(e);
    if (current && el !== current) hide();
  ...
  addEventListener("scroll", hide, { capture: true, passive: true });  // first scroll, no threshold
  addEventListener("resize", hide);
  addEventListener("orientationchange", hide);
  addEventListener("keydown", (e) => {
    if (e.key !== "Escape") return;
    if (current) cooled = current;      // no instant re-show under a still pointer
    hide();
  });
}
...
/* Scatter backgrounds — characters sown across the readymade grids.
   data-bg-scatter="✦ ❋" on a .bg-grid or .bg-dots surface replaces
   roughly one lattice point in ten with one of the given tokens (any
   character or emoji, or a facet icon name); data-bg-scatter-rate
   tunes the frequency — one character per N lattice points, default
   10. Drawn as one repeating SVG tile in the same pattern ink, with
   a small page-colored patch under each character so the plus or dot
   it replaces disappears. The layout is seeded by the input, so a
...
function drawScatterBackground(el) {
  ...
  const isDots = el.classList.contains("bg-dots");
  ...
  const step = isDots ? cell / 2 : cell;   // dots sit every half cell, plus marks every cell
  const ink = patternInk(el, "--bg-ink");
  const paper = getComputedStyle(el).backgroundColor;
  // a 12x12-point tile; the seeded generator keeps the arrangement
  // stable across redraws, so themes re-ink it without reshuffling
  const P = 12;
  let seed = rate;
  for (const ch of raw) seed = (seed * 31 + ch.codePointAt(0)) >>> 0;
  const rand = () => ((seed = (seed * 1664525 + 1013904223) >>> 0) / 2 ** 32);
  const count = Math.max(1, Math.round((P * P) / rate));
  const size = Math.max(9, step * (isDots ? 0.62 : 0.34));
  // the patch hides what the character replaces: a dot, or a whole plus
  ...
  const patch = isDots ? 3 : (plus || 12) / 2 + 1;
  const taken = new Set();
  const parts = [];
...
function initGlyphBackgrounds() {
  ...
  new MutationObserver((muts) => {
    ...
    attributeFilter: ["data-bg-glyph", "data-bg-glyph-size", "data-bg-glyph-scale",
                      "data-bg-scatter", "data-bg-scatter-rate", "style", "data-theme", "data-mode"],
    subtree: true,
  });
}
...
function initTouchPolish() {
  ...
  // (The old 5s .tip-on tooltip peek lived here; tooltips are a real
  // behaviour now — initTooltips owns the bubble on every device.)
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initTouchPolish();
  initTooltips();         // description tooltips: measured placement, hover/focus/tap lifecycle
  initChoiceGrids();
  initSliderScales();
  initMaps();
  initThemeColorMeta();
