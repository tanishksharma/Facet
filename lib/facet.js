/*
  ...
  What lives here (one small named function per behavior):
    ...
    setConfig          facet.set({...}): change theme, mode or any token;
                       ...
                       derive their companions and persist across visits
...
/* Skin overrides — the theme-builder half of facet.set. Six high-level
   keys restyle the live page and persist across visits (localStorage
   "facet-skin", restored at script time so the look holds on return):
     accent1 / accent2 / accent3   re-ink an accent rank; the hover,
...
const skinStore = {
  read() { try { return JSON.parse(localStorage.getItem("facet-skin")) || {}; } catch { return {}; } },
  write(s) {
    try {
      if (Object.keys(s).length) localStorage.setItem("facet-skin", JSON.stringify(s));
      else localStorage.removeItem("facet-skin");
    } catch {}
  },
};
...
/* --------------------------------------------------------------------------
   ...
   every motion profile; only reduced motion quiets them.
     ...
     facetMotion              -> { register(selector, {xMax, yMax} or
                                   ...
                                   accepted); persists to localStorage
                                   motion-mode + parallax-enabled; fires
...
/* (docs mirror — identical to llms.txt · Sheet & menu)
   ...
   What it is: the app's edge panel — a slab floating clear of the screen over a dark scrim, sliding in as one rigid block on a spring that overshoots slightly and eases back. The right edge is the settings side (the default); .sheet-left is the same panel sliding from the LEFT edge — the navigation side — and opening either closes the other. Add .sheet-rise and on phones the panel rises out of the tab bar instead: the bar's own frosted glass, as wide as the bar, standing only as tall as its options need — capped at half the screen so nothing sits out of thumb reach, taller content scrolling inside — mostly opaque so the options read, everything centred, group names in the small eyebrow voice; on wide screens it falls back to its edge-sheet self. Inside, menu groups of thin rows (.menu-item under a .menu-label) and a foot row of square icon toggles (.menu-icon-btn) whose ON state stands raised while OFF sinks into a darker well. What it is for: navigation, actions and app-level controls behind one trigger — any button that aria-controls the sheet opens it; scrim tap and Esc close; the trigger's aria-expanded is managed, a floating trigger stays visible above the open sheet as its close toggle, and a sheet left open never survives the browser's back-forward cache — it closes itself on return. When to use it: every app-shaped product's settings. Press a row: it sinks INTO the panel with a true bevel, never a dark smear. State changes ease — toggles must never jump. */
...
const facetSheet = (sheet, scrim, opts = {}) => {
  ...
  // .sheet-rise on a phone: the panel rises out of the tab bar. The
  // reveal is a clip-path wipe over the panel's FINAL layout — height
  // is never animated, so nothing reflows or squishes mid-motion and
  // the frosted glass renders once, not every frame. Interrupting
  // continues from the current wipe position.
  ...
  const rise = open => {
    ...
    const SHUT = `inset(100% 0 0 0 round ${radius})`;     // wiped down to the bar
  ...
  const set = open => {
    ...
    if (riser()) { rise(open); if (opts.onChange) opts.onChange(open); return; }
...
/* Motion engine — one input, two channels. The device's movement (tilt,
   ...
   init() applies the stored choice (localStorage motion-mode, with
...
const facetMotion = (() => {
  ...
  // missing key means IDLE — never tracking-on uninvited.
  const readStored = () => {
    let m = null, p = null;
    try { m = localStorage.getItem(MKEY); p = localStorage.getItem("parallax-enabled"); } catch {}
  ...
  const store = () => {
    try {
      ...
      localStorage.setItem("parallax-enabled", String(mode === "responsive"));
    } catch {}
  };
...
function initTouchTooltips() {
  ...
  const opensSomething = (el) =>
    el.matches("[aria-controls], [aria-haspopup], summary") || el.closest("summary");
...
function initTabBars() {
  for (const bar of document.querySelectorAll(".tab-bar")) {
    ...
    // centred in view — nobody should have to hunt for where they are
    const centerCurrent = (seg) => {
      const set = bar.querySelector(".tab-set");
      if (!set || set.scrollWidth <= set.clientWidth + 1) return;
      set.scrollLeft = seg.offsetLeft + seg.offsetWidth / 2 - set.clientWidth / 2;
    };
    if (current) {
...
function initSheetTriggers() {
  ...
  for (const trigger of document.querySelectorAll("button[aria-controls]")) {
    ...
    trigger.addEventListener("click", () => {
      if (window.facet.feedback) facet.feedback.tap();
      // one panel at a time: the navigation and settings sheets swap
      for (const other of document.querySelectorAll(".sheet"))
        if (other !== sheet && other.facetSheet && other.facetSheet.isOpen)
          other.facetSheet.close();
      panel.toggle();
    });
  }
}
