...
function drawGlyphBackground(el) {
  ...
  const cell = Math.max(16, parseFloat(el.dataset.bgGlyphSize) || 56);
  ...
  const parts = tokens.map((t, i) => {
    ...
    if (icon) {
      const scale = (cell * 0.42) / 24;
      const off = cell * 0.21;
      return `<g transform="translate(${x - cx + (cx - off)} ${cx - off}) scale(${scale})" ` +
        `fill="none" stroke="${ink}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">${icon}</g>`;
    }
    const safe = t.replace(/&/g, "&amp;").replace(/</g, "&lt;");
    return `<text x="${x}" y="${cx}" font-size="${cell * 0.42}" fill="${ink}" ` +
...
function initGlyphBackgrounds() {
  ...
  new MutationObserver((muts) => {
    ...
    attributes: true,
    attributeFilter: ["data-bg-glyph", "data-bg-glyph-size", "data-theme", "data-mode"],
    subtree: true,
  });
}
