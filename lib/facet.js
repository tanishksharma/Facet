...
/* (docs mirror — identical to llms.txt · Subscribe)
   ...
   What it is: the behaviour behind the .subscribe well. On submit it
   validates the email, sets data-ok on the .subscribe-msg line, and
   dispatches a facet:subscribe CustomEvent (detail: { email, form }) so
   the project does the network call — the library never owns an endpoint.
   The event is cancelable: call event.preventDefault() to take over the
   status line yourself (show your own "sending…" then result); leave it
   and the well shows a default confirmation. With JS off the form posts
   natively, so give it a real action if you have no listener. */
/* --------------------------------------------------------------------------
   SUBSCRIBE
