...
/* --------------------------------------------------------------------------
   ...
   every motion profile; only reduced motion quiets them.
     ...
     facetMotion              -> { register(selector, {xMax, yMax} or
                                   ...
                                   register/setMode/cycle boot the
                                   engine on demand; init is idempotent;
                                   register dedups per element+channel
...
/* (docs mirror — identical to llms.txt · Parallax & idle motion)
   ...
   nothing else drives (modes: off | cursor | tilt | idle). The engine
   ...
   jitter, no easing lag — and the moment the pointer or device stops,
   the vector eases back to centre (in and out, about half a second), so
   nothing rests jammed off-centre. Idle rests at that same centre and, every few seconds,
   takes one fast smooth loop — down out of the centre, once around, and
   back up into it (about 1.1s) — so idle starts and ends exactly where
   everything else rests; gated touch devices land here by default. The same engine's light channel drives the shine
...
/* Motion engine — one input, two channels. The device's movement (tilt,
   ...
   two channels:
     ...
     modes: "off"     nothing moves; shine rests at its CSS default
            ...
            "idle"    no input available: one fast smooth loop out of
                      the centre and back (~1.1s), a ~3s rest, repeat
   ...
   granted), cursor elsewhere, off under prefers-reduced-motion or a
   page-level data-motion="off". The engine boots lazily: init() is
   idempotent, and register()/setMode()/cycle() call it on demand, so a
   page with no declarative movers still gets a live engine the moment
   one registers at runtime or a settings control cycles the mode. */
const facetMotion = (() => {
  const parts = [];
  let mode = "off";
  let target = { x: 0, y: 0 };
  ...
  const frame = now => {
    // the idle life: the vector rests at CENTRE — exactly where cursor
    // and tilt return to — and every few seconds takes one fast smooth
    // loop: down out of the centre, once around, and back up into it
    // (~1.1s), then rests ~3s. r = sin(pi*e) grows and shrinks the
    // radius while the angle makes a full turn, so the route is one
    // continuous curve with no corners, eased in and out.
    if (mode === "idle") {
    ...
    } else if (mode === "cursor" || mode === "tilt") {
      // the moment the real world stops, ease the vector home — in and
      // out, no lag, no exponential crawl — so nothing rests jammed
      // off-centre; any fresh input cancels the journey instantly
  ...
  const api = {
    ...
    setMode(want) {
      ...
      return setMode(want);             // a page with no declarative movers
    },
    cycle() {
      if (!booted) api.init();
...
/* (docs mirror — identical to llms.txt · Description tooltip)
   ...
   lands anywhere else, or the same element is tapped again. A tablet's
