...
/* (docs mirror — identical to llms.txt · Parallax & idle motion)
   ...
   off-centre. Idle rests at that same centre and, every few seconds,
...
const facetMotion = (() => {
  ...
  const RETURN_DELAY = 450;          // ms of stillness before centring begins
  const RETURN_RATE = 0.92;          // the per-frame drift home (~half a second)
  ...
  const frame = now => {
    ...
    if (mode === "idle") {
      ...
      // the real world went still: bring the vector home quickly, so
