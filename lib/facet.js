...
/* (docs mirror — identical to llms.txt · Parallax & idle motion)
   ...
   facet.motion.setMode("tilt") asks), or the engine's idle drift where
   nothing else drives (modes: off | cursor | tilt | idle — in idle the
   vector travels the rim in one slow continuous circle, so motion
   keeps living on a still desk; gated touch devices land here by
   default). The same engine's light channel drives the shine effect
   (next entry) from the same vector. The idle-* classes (.idle-float,
   .idle-sway, .idle-pulse) give resting elements a slow pulse; they
   animate the separate translate/rotate/scale properties so they
   compose with the engine's transform instead of colliding — the
...
/* Motion engine — one input, two channels. The device's movement (tilt,
   ...
   two channels:
     ...
     modes: "off"     nothing moves; shine rests at its CSS default
            ...
            "idle"    no input available: the vector travels the rim in
                      one slow continuous circle (about 14s a lap)
...
const facetMotion = (() => {
  ...
  let idleLast = null, idleAngle = 2.1;
  const STIFF = 0.10, DAMP = 0.80;
  const IDLE_SPEED = (Math.PI * 2) / 14000;  // one lap of the rim per ~14s
  const IDLE_R = 0.7;
  const clamp1 = v => Math.max(-1, Math.min(1, v));

  const frame = now => {
    // the idle drift: with no live input the vector travels the rim in
    // one slow continuous circle, so parallax pieces orbit and the
    // light wheels around — motion keeps living on a still desk
    if (mode === "idle") {
      idleAngle += (now - (idleLast ?? now)) * IDLE_SPEED;
      idleLast = now;
      target.x = Math.cos(idleAngle) * IDLE_R;
      target.y = Math.sin(idleAngle) * IDLE_R;
    }
    let settled = true;
