...
const facetSheet = (sheet, scrim, opts = {}) => {
  ...
  // .sheet-rise on a phone: the panel grows UP out of the tab bar
  // instead of sliding from an edge — same engine handle, different body
  ...
  const rise = open => {
    ...
    if (open) {
      const h = sheet.offsetHeight;     // natural height, capped by the stylesheet
      sheet._rise = sheet.animate(
        [{ height: "0px", paddingTop: "0px", paddingBottom: "0px" }, { height: h + "px" }],
        { duration: 420, easing: EASE });
      sheet._rise.onfinish = () => { sheet._rise = null; };
    } else {
      const h = sheet.offsetHeight;
      sheet.style.visibility = "visible";   // stay visible while shrinking
      sheet._rise = sheet.animate(
