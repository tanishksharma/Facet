...
const facetMotion = (() => {
  ...
  const frame = now => {
    ...
    if (mode === "idle") {
      ...
      // the lap eases in and out (cosine ease): it leaves the rest
      // gently, sweeps fast through the middle, and settles into the
      // next rest instead of stopping dead
      const e = (1 - Math.cos(Math.PI * p)) / 2;
      const a = IDLE_BASE + e * Math.PI * 2;
