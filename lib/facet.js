...
/* (docs mirror — identical to llms.txt · Parallax & idle motion)
   ...
   back up into it (about 0.8s) — so idle starts and ends exactly where
   ...
   two channels:
     ...
     modes: "off"     nothing moves; shine rests at its CSS default
            ...
            "idle"    no input available: one fast smooth loop out of
                      the centre and back (~0.8s), a ~3s rest, repeat
...
const facetMotion = (() => {
  ...
  const IDLE_LAP = 800, IDLE_REST = 3000;   // one fast loop, then a rest
  const IDLE_R = 0.7;
  ...
  const frame = now => {
    ...
    // (~0.8s), then rests ~3s. r = sin(pi*e) grows and shrinks the
