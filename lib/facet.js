...
/* (docs mirror — identical to llms.txt · Field actions)
   ...
   cursor), Select all, and Undo (step back through the field's own history).
...
function initFieldActions() {
  for (const field of document.querySelectorAll("[data-field-actions]")) {
    ...
    // Undo history: snapshot at focus and before each mutation, deduped
    const history = [];
    const snapshot = () => {
      if (history[history.length - 1] !== input.value) history.push(input.value);
    ...
    actions.innerHTML =
      ...
      '<summary class="btn btn-ghost btn-icon btn-small" aria-label="Field actions" data-tip="Copy, paste, select all, undo">' +
      '<svg width="18" height="18" data-icon="more" aria-hidden="true"></svg></summary>' +
      '<div class="dropdown-menu">' +
      '<button type="button" data-field-do="copy" data-event="field-copy">Copy</button>' +
      '<button type="button" data-field-do="paste" data-event="field-paste">Paste</button>' +
      '<button type="button" data-field-do="select" data-event="field-select">Select all</button>' +
      '<button type="button" data-field-do="undo" data-event="field-undo">Undo</button>' +
      '</div></details>';
    ...
    for (const item of actions.querySelectorAll("[data-field-do]")) {
      item.addEventListener("click", async () => {
        const act = item.dataset.fieldDo;
        if (act === "copy") {
          ...
          if (history.length) { commit(history.pop()); toggleClear(); }
          input.focus();
          feedback("tap");
        }
      });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initThemeColorMeta();
  initSteppers();
  initFieldActions();     // [data-field-actions] fields: inject clear + copy/paste/select/undo menu
  initDialogTriggers();
