...
/* (docs mirror — identical to llms.txt · Appearance panel)
   ...
   theme's own look; and a [data-shortcuts] slot is filled with the page's
...
/* Rail dots — a dot per card under a horizontal scroller.
   data-rail-dots on any horizontal scroller (a .card-row shelf, a
   carousel) grows a row of dots beneath it, one per direct child: the
   dot for the card most in view fills in (aria-current="true"), and
   clicking a dot scrolls its card to centre. Self-wiring and guarded
   against double-init; the rail scrolls and drags on its own, so this
   is a wayfinding aid, not the scroll mechanism, and with JS off no dots
   appear. Re-run facet.railDots() after inserting a late rail. */
const railDotsWired = new WeakSet();   // guard without leaving an attribute in the DOM (so a copied snippet still wires)
function initRailDots(root = document) {
  for (const rail of root.querySelectorAll("[data-rail-dots]")) {
    if (railDotsWired.has(rail)) continue;   // never wire twice
    railDotsWired.add(rail);
    ...
    dots.setAttribute("data-demo-chrome", "");   // injected: not snippet material
    const railLabel = rail.getAttribute("aria-label");
    dots.setAttribute("aria-label", railLabel ? `${railLabel} positions` : "Positions");
    ...
    // Mark the dot of whichever card is most in view. IntersectionObserver
    // rooted on the rail; if it is unavailable the dots still click.
    if (typeof IntersectionObserver === "function") {
      const ratios = new Map();
      const mark = () => {
        let best = -1, bestI = 0;
        cards.forEach((c, i) => { const r = ratios.get(c) || 0; if (r > best) { best = r; bestI = i; } });
        marks.forEach((m, i) => {
          if (i === bestI) m.setAttribute("aria-current", "true");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initRailDots();         // [data-rail-dots] scrollers get a dot per card, current in view marked
...
window.facet = {
  ...
  railDots: initRailDots,          // (re)wire [data-rail-dots] scrollers after inserting markup
