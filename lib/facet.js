/*
  ...
  The motion pack landed: data-parallax="depth" wires elements to
  facet.motion (pointer, scroll momentum, permission-gated tilt), and
  the idle-* classes breathe on the composable translate/rotate/scale
...
/* Appearance is three-way: "auto" follows the OS, "light" and "dark" are
   ...
   localStorage; while on auto, a live prefers-color-scheme listener keeps
   the page in step with the OS. facet.scheme exposes get/set/cycle; the
   settings icon-row cycles Light -> Dark -> Auto through it. data-mode on
   <html> stays the render attribute the CSS reads. */
const SCHEME_KEY = "facet-scheme";
const schemeStore = {
  get() { try { return localStorage.getItem(SCHEME_KEY); } catch { return null; } },
  set(v) { try { v ? localStorage.setItem(SCHEME_KEY, v) : localStorage.removeItem(SCHEME_KEY); } catch { /* private mode */ } },
};
...
/* --------------------------------------------------------------------------
   ...
   facet.location stays null. If the visitor has already granted
   geolocation permission elsewhere in the app, coordinates upgrade to
   GPS accuracy silently. The library never prompts for permission.
   -------------------------------------------------------------------------- */
...
function facetPager(snap, opts = {}) {
  ...
  let cur = 0, raf = null, x = 0, v = 0, target = 0, onGlide = null;
  ...
  const frame = () => {
    ...
    if (Math.abs(target - x) < 0.5 && Math.abs(v) * DT < 0.5) {
      snap.scrollTop = x = target; v = 0; raf = null; if (onGlide) onGlide(); return;
    }
  ...
  /* Optional whole-page scroll gauge: a draggable right-edge lane whose
     ...
     is its inner scroll, the trailing vh is the outer glide to the next.
...
/* Declarative parallax: any element carrying data-parallax="depth"
   (px of maximum drift, default 8) registers with the engine, and the
   engine starts — desktop pointer drives it, scroll velocity kicks
   it, and on gyro devices without a permission gate it follows tilt.
   data-motion="off" and prefers-reduced-motion keep everything still.
   The exclusion law is structural here: the engine owns style
   .transform, so never put data-parallax on elements with their own
   transform physics (velvet lift/press, the pager). The idle-*
   classes are safe company — they animate the separate translate/
   rotate/scale properties, which compose instead of colliding. */
...
/* Settings icon-row controls, self-wired by data-control on any
   .menu-icon-btn: "sounds" and "haptics" flip facet.feedback switches,
   "motion" cycles facet.motion (Off/Cursor/Tilt), "appearance" cycles
   facet.scheme (Light/Dark/Auto). Each writes its .menu-value word,
   marks data-off for the dimmed OFF state, and plays feedback.toggle().
   A four-button sheet foot needs zero app JS. */
function initAppControls() {
  ...
  for (const btn of document.querySelectorAll(".menu-icon-btn[data-control]")) {
...
/* Nav menu: upgrades a .nav-menu's <details> into a floating overlay —
   ...
   JavaScript; this only adds the polish. prefers-reduced-motion drops
   the stagger. Idempotent: the data-enhanced guard lets a page re-run it
   after inserting new markup. The Settings gear beside the pill is a
...
function initNavMenu() {
  ...
  for (const nav of document.querySelectorAll(".nav-menu")) {
    ...
    const details = nav.querySelector("details.nav-menu-panel");
    const summary = details && details.querySelector("summary");
    if (!details || !summary) continue;
    const items = [...details.querySelectorAll(".nav-menu-items > *")];
    nav.dataset.enhanced = "true";

    // one scrim per menu, at the end of <body> (reading-order law: chrome
    // is authored last so a top-to-bottom copy sweeps content first)
    ...
    scrim.addEventListener("click", () => { details.open = false; });
...
window.facet = {
  version: "0.2.0",   // the app-kit ergonomics release
  set: setConfig,
