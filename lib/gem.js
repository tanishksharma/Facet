 ...
 *    • Fire          — brief flashes on random
 *                      facets, both front and back, but never the big central
 *                      table. On by default: multicolor for diamond,
 *                      white for every other stone.
...
(function (global) {
  ...
  // Fire is on by default: multicolor for diamond, white for every other stone.
  ...
  function paintFire(g) {
    ...
    for (let budget = 3; g.fires.length < want && budget-- > 0;) {
      ...
      g.fires.push({ back, fi, life: 5 + Math.floor(Math.random() * 6), color: g.isDiamond ? FIRE[Math.floor(Math.random() * FIRE.length)] : '#fff' });
    }
    const fm = {}, bm = {};
  ...
  Gem.prototype.build = function (options) {
    ...
    const def = { highlight: true, sheen: true, scintillation: true, fire: true, idle: false };
