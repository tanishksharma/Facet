...
- Frameworks like React exist to help humans manage complexity while coding: component reuse, state, big teams.
- That trade cost the web its nature: build steps, npm, deep dependency trees, minified output no human can read.
...
React made the web complex so developers could build. AI does the building now, so we go back to a clean web and let anyone build.
...
- React still earns its keep for big stateful apps and huge teams.
- For the other 80 percent, sites, docs, dashboards, decks, PWAs, most MVPs, it is overkill.
- Facet owns the 80 percent.
