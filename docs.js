...
// a cleaned clone: controls living on the demo surface (data-demo-chrome)
// are docs furniture, the data-bg-surface hook is this page's own wiring,
// and the background engines' drawn custom properties are regenerated by
// facet.js from the data attributes — none of it is snippet material
function cleanDemoClone(el) {
  ...
  for (const node of [clone, ...clone.querySelectorAll("[style], [data-bg-surface]")]) {
    ...
    if (node.style) {
      for (const prop of ["--bg-glyph", "--bg-glyph-tile", "--bg-scatter", "--bg-scatter-tile"]) {
...
function renderSnippet(article) {
  ...
  // a wrapper marked data-demo-unwrap is a docs grouping (a variant
  // section holding a preview plus its controls) — the snippet takes its
  // real children, not the wrapper itself, so the copied code is the piece
  const serialize = (el) => el.hasAttribute("data-demo-unwrap")
    ? [...el.children]
        .filter(c => !c.hidden && !c.hasAttribute("data-demo-chrome"))
        .map(c => cleanDemoClone(c).outerHTML)
        .join("\n\n")
  ...
  for (const chip of document.querySelectorAll(".element .chip:not([data-config-key])")) {
    chip.addEventListener("click", () => {
      ...
      if (chip.dataset.chipGroup !== undefined) {
        ...
        const attr = chip.dataset.chipAttr;
        const value = chip.dataset.chipAttrValue ?? "";   // e.g. aria-busy="true"
